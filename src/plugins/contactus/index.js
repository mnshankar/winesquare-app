    ...
    createData() {
        ...
        if (instantConfig.email && instantConfig.email != '') {
            instantConfig.email.map(item => {
                contactItems.push({
                    link: 'mailto:' + item,
                    title: (instantConfig.email.length == 1) ? Identify.__('Email') + ': ' + item : Identify.__('Email') + item,
                    icon: 'ios-mail'
                })
            })
        }
        ...
        if (instantConfig.phone && instantConfig.phone != '') {
            instantConfig.phone.map(item => {
                contactItems.push({
                    link: 'tel:' + item,
                    title: (instantConfig.phone.length == 1) ? Identify.__('Call') + ': ' + item : Identify.__('Call') + item,
                    icon: 'ios-call'
                })
                return null
            })
        ...
        if (instantConfig.message && instantConfig.message != '') {
            instantConfig.message.map(item => {
                contactItems.push({
                    link: 'sms:' + item,
                    title: (instantConfig.message.length == 1) ? Identify.__('Message') + ': ' + item : Identify.__('Message') + item,
                    icon: 'ios-chatboxes'
                })
                return null
            })
        ...
        if (instantConfig.website && instantConfig.website != '') {
            contactItems.push({
                ...
                title: Identify.__('Website') + ': ' + instantConfig?.website,
                icon: 'ios-globe'
            })
        }
        return contactItems;
