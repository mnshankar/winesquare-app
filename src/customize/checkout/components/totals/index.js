...
class Totals extends React.Component {
    ...
    renderView() {
        ...
        if (data) {
            ...
            if (this.tax_cart_display_grandtotal === 1) {
                grand_total = (<View style={this.props.styleTwoRowPrice}>
                    {data.grand_total_excl_tax ?
                        <View style={this.props.styleOneRowPrice}>
                            <Text style={[this.props.styleLabelTotal, { color: Identify.theme.price_color, fontFamily: material.fontBold }]}>{Identify.__('Grand Total')} ({Identify.__('Excl. Tax')})</Text>
                            <Format price={data.grand_total_excl_tax} style={[this.props.stylePriceTotal, { color: Identify.theme.price_color, fontFamily: material.fontBold }]} type={this.type} currency_symbol={this.currency_symbol} />
                        </View> : null
                    }
                    {data.grand_total_incl_tax ?
                        <View style={this.props.styleOneRowPrice}>
                            <Text style={[this.props.styleLabelTotal, { color: Identify.theme.price_color, fontFamily: material.fontBold }]}>{Identify.__('Grand Total')} ({Identify.__('Incl. Tax')})</Text>
                            <Format price={data.grand_total_incl_tax} style={[this.props.stylePriceTotal, { color: Identify.theme.price_color, fontFamily: material.fontBold }]} type={this.type} currency_symbol={this.currency_symbol} />
                        </View> : null
                    }
                </View>);
                ...
                grand_total = (
                    <View style={this.props.styleOneRowPrice}>
                        <Text style={[this.props.styleLabelTotal, { color: Identify.theme.price_color, fontFamily: material.fontBold }]}>{Identify.__('Grand Total')}</Text>
                        <Format price={data.grand_total_incl_tax} style={[this.props.stylePriceTotal, { color: Identify.theme.price_color, fontFamily: material.fontBold }]} type={this.type} currency_symbol={this.currency_symbol} />
                    </View>
                );
            }
            ...
            let qtyItem = this.props.data.quoteitems && this.props.data.quoteitems.cart_total ? this.props.data.quoteitems.cart_total + ' ' + Identify.__('items') : ''
            return (
                <View>
                    <View style={{ backgroundColor: '#F2F2F2', padding: 15 }}>
                        <Text style={{ fontFamily: material.fontBold, marginBottom: 10 }}>{Identify.__("Summary")} ({qtyItem})</Text>
                        {custom}
                        {subtotal}
                        {shipping}
                        {discount}
                    ...
                    <View style={{ backgroundColor: '#F2F2F2', padding: 15, marginTop: 10 }}>
                        {grand_total}
                    </View>
                </View>
            );
...
Totals.defaultProps = {
    ...
    styleLabel: {},
    stylePrice: {},
    styleLabelTotal: { color: material.priceColor, flex: 1, textAlign: 'left' },
    stylePriceTotal: { color: material.priceColor, flex: 1, textAlign: 'right' },
    ...
    styleMargin: { marginBottom: 20, marginTop: 20 },
    type: 0,
