...
import { View, Keyboard, ScrollView, KeyboardAvoidingView, Alert, Platform } from 'react-native'
...
class Checkout extends SimiPageComponent {
    constructor(props) {
        ...
        this.state = {
            ...this.state,
            delivery_instructions: this.props.delivery_instructions
        };
        ...
        this.isPayment = this.props.navigation.getParam('isPayment') ? this.props.navigation.getParam('isPayment') : null;
        this.list = null;
        this.totals = null;
        ...
        this.onSaveStorePickup = this.onSaveStorePickup.bind(this);
    ...
    componentWillMount() {
        if (this.props.showLoading.type === 'none' && !this.isPayment) {
            this.props.storeData('showLoading', { type: 'full' });
        }
        if (this.isPayment) {
            this.totals = this.props.data.order.total;
        }
    }
    ...
    onSaveStorePickup(location) {
        this.props.storeData('showLoading', { type: 'dialog' });
        this.shippingAddressParams = location
        let bodyData = {
            s_method: {
                method: 'instore_pickup'
            },
            b_address: this.billingAddressParams,
            s_address: {
                ...this.shippingAddressParams,
    ...
    onSaveMethod(bodyData, isInitRequest = false) {
        if (bodyData.s_method && bodyData.s_method.method == 'instore_pickup') {
            NavigationManager.openPage(this.props.navigation, 'StorePickup', {
                onSaveStorePickup: this.onSaveStorePickup
            });
        } else {
            if (!isInitRequest) {
                console.log("127");
                ...
                //             ...this.shippingAddressParams,
                //             caf: {
                //                 delivery_instructions: this.state.delivery ? this.state.delivery : ''
                //             }
                //         }
                //     }
                // }
                bodyData = {
                    ...
                    s_address: {
                        ...this.shippingAddressParams,
                        caf: {
                            delivery_instructions: this.state.delivery_instructions ? this.state.delivery_instructions : ''
                        }
                    }
                }
            }
    ...
    onPlaceOrder(params = null) {
        if (!this.isPayment) {
            ...
            NavigationManager.openPage(this.props.navigation, 'Checkout', {
                mode: this.mode,
                shippingAddressParams: this.shippingAddressParams,
                billingAddressParams: this.billingAddressParams,
                isPayment: true,
            });
            return;
        } {
            ...
            default:
                ...
                if (!checkCustomPayment) {
                    if (order.hasOwnProperty('notification') && !Identify.isEmpty(order.notification) && Identify.TRUE(order.notification.show_popup)) {
                        ...
                        this.props.storeData('delivery_instructions', '');
                        NavigationManager.clearStackAndOpenPage(this.props.navigation, 'Thankyou', {
                            invoice: order.invoice_number,
                            payment: this.selectedPayment.title,
                            mode: this.mode,
                        });
                    }
                }
    ...
    renderPhoneLayout() {
        ...
        return (
            ...
            <Container style={{ backgroundColor: material.appBackground }}>
                <ScrollView
                    ref={this.setScrollViewRef} showsVerticalScrollIndicator={false}>
                    <Content>
                        <View style={{ paddingBottom: Platform.OS == 'android' ? 20 : material.isIphoneX ? 100 : 80 }}>
