...
class Thankyou extends SimiPageComponent {
  ...
  componentDidMount() {
    ...
    let data = {};
    data['event'] = 'thankyou_action';
    data['action'] = 'purchase';
    data['order_review_data'] = this.props.order_review_data.order;
    data['invoice'] = this.props.navigation.getParam('invoice');
  ...
  renderPhoneLayout() {
    AppStorage.removeData(['delivery_instructions']);
    return (
...
const mapStateToProps = (state) => {
  return {
    order_review_data: state.redux_data.order_review_data,
  };
}
